...
namespace FalseTerrarianTest.Projectile
{
	public class ExampleLastPrismHoldout : ModProjectile
	{
		...
		{
			Projectile.damage = (int)player.GetTotalDamage(DamageClass.Magic).ApplyTo(player.HeldItem.damage);
		}
		...
		private void FireBeams()
		{
			...
			int uuid = Terraria.Projectile.GetByUUID(Projectile.owner, Projectile.whoAmI);

			int damage = Projectile.damage;
			float knockback = Projectile.knockBack;
			for (int b = 0; b < NumBeams; ++b) {
				Terraria.Projectile.NewProjectile(Projectile.GetSource_FromThis(), Projectile.Center, beamVelocity, ModContent.ProjectileType<ExampleLastPrismBeam>(), damage, knockback, Projectile.owner, b, uuid);
			}
