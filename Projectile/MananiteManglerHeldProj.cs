...
{
	class MananiteManglerHeldProj : ModProjectile
	{
		...
		public override void AI()
		{
			...
			UpdateAim(rrp);
			Animate(Main.projFrames[Projectile.type], 2);
			DebugParticles();
		...
		private void UpdateAim(Vector2 source)
		{
			...
			//Position projectile by 70 units away from player. Value obtained from predetermined value in sprite (how far away is the dummy from the center)
			Projectile.position = Projectile.position + aim * 70;
			Projectile.rotation = aim.ToRotation();
			Projectile.velocity = aim;
		}
		private void PlaySoundRepeat(int interval)
		{
		...
		private void Particles(Vector2 source, int width, int height, int intensity = 1, int rate = 30)
		{
			source.X -= width / 2;
			source.Y -= height / 2;
			if (intensity <= 1)
		...
		private void DebugParticles()
		{
			for (int i = 0; i < 5; ++i)
			{
				Vector2 upBorder = Projectile.position;
				Vector2 downBorder = Projectile.position;
				Vector2 leftBorder = Projectile.position;
				Vector2 rightBorder = Projectile.position;
				upBorder.Y -= Projectile.height / 2;
				downBorder.Y += Projectile.height / 2;
				rightBorder.X -= Projectile.width / 2;
				leftBorder.X += Projectile.width / 2;
				Dust.NewDust(upBorder, Projectile.width, 0,
				55, 0f, 0f, 0, Color.Red, 1.0f);
				Dust.NewDust(downBorder, Projectile.width, 0,
				55, 0f, 0f, 0, Color.Blue, 1.0f);
				Dust.NewDust(rightBorder, 0, Projectile.height,
				55, 0f, 0f, 0, Color.Green, 1.0f);
				Dust.NewDust(leftBorder, 0, Projectile.height,
				55, 0f, 0f, 0, Color.Yellow, 1.0f);
			}
		}
	}
}