...
{
	class ProminenceSlash : ModProjectile
	{
		...
		public override void OnHitNPC(NPC target, int damage, float knockback, bool crit)
		{
			...
			//Terraria.Projectile.NewProjectileDirect(Projectile.GetSource_FromThis(), target.position, target.velocity, ProjectileID.SolarWhipSwordExplosion, damage, knockback, Projectile.owner);
		}
		...
		private void Animate(int frames, int ticksPerFrame)
		{
			if (++Projectile.frameCounter >= ticksPerFrame)
			{
				...
				if (Projectile.frame == 1 || Projectile.frame == 6 || Projectile.frame == 11 || Projectile.frame == 13)
				{
					PlaySoundOnce();
